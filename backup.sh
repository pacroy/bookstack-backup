...
kubectl exec --context "$KUBE_CONTEXT" --namespace="$WIKI_NAMSPACE" --container="$BOOKSTACK_CONTAINER" "$BOOKSTACK_POD_NAME" -- bash -c "tar -czf - -C /var/www/bookstack/public/uploads *" > ./backup/uploads.tgz

printf "Copying BookStack Storage from %s ...\n" "$BOOKSTACK_POD_NAME"
kubectl exec --context "$KUBE_CONTEXT" --namespace="$WIKI_NAMSPACE" --container="$BOOKSTACK_CONTAINER" "$BOOKSTACK_POD_NAME" -- bash -c "tar -czf - -C /var/www/bookstack/storage uploads" > ./backup/storage.tgz
