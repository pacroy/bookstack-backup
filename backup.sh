...
kubectl exec -it --context $KUBE_CONTEXT --namespace=$WIKI_NAMSPACE $BOOKSTACK_POD_NAME -- bash -c "rm -f /var/www/bookstack/storage.tgz && cd /var/www/bookstack/storage/ && tar -cvzf /var/www/bookstack/storage.tgz uploads | wc -l | xargs -i echo {} 'file(s) archived' && exit"
